...
clear
CPU_MODEL=\$(lscpu | grep 'Model name' | head -n1 | awk -F: '{print \$2}' | sed 's/^ //')
UPTIME=\$(uptime -p)
...
# Run the function to add banner to bashrc
add_banner_to_bashrc

# Clear screen again before showing banner immediately
clear
